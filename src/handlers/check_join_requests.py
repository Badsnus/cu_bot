...
async def handle_user_joining_via_invite_link(
    ...
    if not can_user_join:
        await request.answer_pm(
            '<b>С этого юзернейма уже зашел другой пользователь.'
            'Или вас исключили из списков\n\n</b>'
            '<code>Если вы считаете, что это ошибка '
            '- напишите администратору.</code>',
        )
        return
