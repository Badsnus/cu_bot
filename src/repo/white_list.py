...
class WhiteListRepo:
    ...
    async def get_by_chat(self, chat_id: int) -> Sequence[WhiteList]:
        return (
            await self.session.scalars(
                select(WhiteList).where(WhiteList.chat_id == chat_id),
            ),
        ).all()

    async def remove_usernames(self,
                               chat_id: int,
    ...
    async def check_is_user_in_white_list(self,
                                          chat_id: int,
                                          username: str) -> bool:
    ...
    async def get_user_in_white_list(self,
                                     chat_id: int,
                                     username: str) -> WhiteList:
        return await self.session.scalar(
            select(WhiteList).filter(
                and_(WhiteList.chat_id == chat_id,
                     WhiteList.username == username,
                     ),
            ),
        )
